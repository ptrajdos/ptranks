            ...
            for n_methods in [1, 3, 5]:
                for n_runs in [1, 3, 5, 10]:
                    for _ in range(exp_repetitions):
                        ...
                        self.assertFalse(any(np.isnan(ranks)),
                                         "Ranks contain NaNs")
                        self.assertFalse(any(np.isinf(ranks)),
                                         "Ranks with infinite values")
                        self.assertTrue(ranks.shape == (
                            n_methods,), "Wrong size of rank array")

                        desired_rank_sum = n_methods * (n_methods + 1.0)/2.0
                        rank_sum = np.sum(ranks)

                        self.assertTrue(np.allclose(
                            rank_sum, desired_rank_sum), "Wrong ranks sum")
